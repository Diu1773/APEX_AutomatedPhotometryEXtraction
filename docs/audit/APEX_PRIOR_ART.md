...
| 프레임 QC 게이트 (depth-cost, 초과검출) | **depth-cost: DES `t_eff` (Neilsen+ 2016) 가 직접 선례** — 시잉·하늘·투명도로 등가 노출 스케일을 계산해 노출을 게이트, Δm ≈ 1.25 log₁₀ t_eff 로 같은 물리량. 초과검출: 두 차례 표적 검색에서 프레임 단위 야간상대 검출수 게이트의 선례 미발견 | **depth-cost 신규성 철회 → 인용·적응 서술** / **초과검출 조건부 주장 유지** | depth-cost 는 "서베이 운영 QC(t_eff) 개념의 프레임 단위·등급 표현·소형 천문대 적응"으로 쓴다. 초과검출은 미발견≠부재이므로 "we found no published equivalent" 수준으로 |
| 구경보정 워크플로 | photutils 프리미티브 + 각 파이프라인의 자체 정책 | **워크플로 기여** | 감사 문서 문구 그대로: "does not replace photutils" |
| **PSF 측광 (step8)** *(2026-08-12 추가 — 이 표에 없었다)* | **DAOPHOT/ALLSTAR (Stetson 1987, PASP 99, 191)** 이 혼잡 시야 PSF 측광의 정본. **DOLPHOT** (Dolphin 2000) 이 그 계열의 현대 표준. 커널은 photutils `EPSFBuilder`/`PSFPhotometry` | **주장 불가 (알고리즘)** / **워크플로 기여 (정책층)** | 커널도 알고리즘도 선행 존재. APEX 고유는 **오염인지 기준성 선택 · 포위에너지 90 % 자동 적합창 · 잔차 패스 · 국소 그룹화**뿐이다. 2026-08-12 에 ALLSTAR 와 처음 맞대 봤고(`validation/psf_engines/`) **아직 결론 없음** — 주입 25 개, 통제 후 산포 비 1.35 배 |
...
| **Stetson 1987, PASP 99, 191 (DAOPHOT)** · Dolphin 2000 (DOLPHOT) | **혼잡 시야 PSF 측광의 정본.** 원고 본문은 이미 `\citep{stetson1987}` 를 달고 있으나 **이 표에는 빠져 있었다** (2026-08-12 추가) |

> **2026-08-12 — 왜 빠져 있었는지.** step8 은 이 표에도, 성적표 표에도 없었고,
> IRAF 대조 하네스도 `phot`(조리개)만 불렀다. **APEX 에서 두 번째로 큰 자작
> 엔진(8,589 줄)이 감사 표면 세 곳을 전부 통과해 버렸다.** 한 문서의 누락이
> 아니라 계통적 사각지대였다. 새 엔진을 감사에 넣을 때는 이 표 · 성적표 ·
> 대조 하네스 셋을 함께 확인할 것.
...
3. ~~depth-cost·초과검출 게이트의 문헌 선례~~ → **2026-08-10 해소.**
   depth-cost 는 DES t_eff(Neilsen+ 2016)가 직접 선례라 신규성 철회, 인용
   필수. 초과검출은 두 차례 표적 검색에서 미발견 — 조건부 주장 유지.
