...
**관측소마다 필터를 자기 방식으로 적습니다.** LCO 는 g′ 를 `gp` 로 쓰고, 어떤
곳은 `SDSS-G`, 어떤 곳은 `Bessell-V`, 어떤 곳은 필터휠 번호를 그대로 씁니다.
APEX 는 흔한 철자를 기본으로 알고 있지만 세상의 모든 이름을 담을 수는 없으므로,
**여기에 직접 적어 넣을 수 있습니다.**
...
**왼쪽은 헤더의 `FILTER` 값 그대로**입니다(대소문자는 안 가립니다).
**오른쪽은 APEX 의 정본 키**입니다 — Johnson 은 대문자 `B V R I`, SDSS 는
소문자 `g r i z`, 협대역은 `Ha` `OIII` 처럼 첫 글자만 대문자입니다.

**여기 적은 것이 기본표를 이깁니다.** 어떤 관측소가 APEX 가 아는 이름을 다른
뜻으로 쓴다면 그 관측소가 옳습니다.

**적지 않은 이름은 그대로 흘러갑니다.** 오류가 나지는 않지만, 그 필터는 「어느
체계인지 모르는 필터」로 다뤄져 SDSS·Johnson 을 전제한 변환에서 빠집니다.
필터가 결과에 안 잡히면 여기를 먼저 보세요.

**대역이 다르면 묶지 마세요.** 예를 들어 z-short(`zs`)는 z′ 와 다른 대역입니다.
같은 키로 묶으면 편하지만 없는 등가를 주장하는 셈이 됩니다. 위 예시에 `zs` 가
들어 있는 것은 **그렇게 하고 싶다면 할 수 있다**는 뜻이지 권한다는 뜻이 아닙니다.
