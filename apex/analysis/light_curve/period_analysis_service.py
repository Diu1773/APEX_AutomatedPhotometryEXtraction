...
def resolve_multinight_period(
    ...
    """Compatibility wrapper around sampling-window candidate analysis.

    The legacy implementation trusted the longest single night as an
    alias-free reference.  That is unsafe when the night covers too little of
    a cycle.  The wrapper now ranks candidates generated from the measured
    spectral window and exposes the conservative status in its return value.
    """
    ...
    if "error" in ls_result:
        return {
            "period": float("nan"),
            "freq_cd": float("nan"),
            "naive_period": float("nan"),
            "reference_freq_cd": float("nan"),
            "reference_night": None,
            "was_aliased": False,
            "n_nights": 0,
            "status": "INSUFFICIENT",
    ...
    analysis = analyze_period_aliases(
        ...
        max_period,
        harmonics=2,
        max_candidates=max(int(n_top), 3),
        n_injections=0,
    )
    resolved_f = float(analysis.get("adopted_freq_cd", np.nan))
    return {
        "period": float(analysis.get("adopted_period", np.nan)),
        "freq_cd": resolved_f,
        "naive_period": float(analysis.get("naive_period", np.nan)),
        "reference_freq_cd": float("nan"),
        "reference_night": None,
        "was_aliased": bool(analysis.get("was_aliased", False)),
        "n_nights": int(analysis.get("n_nights", 0)),
        "status": str(analysis.get("status", "AMBIGUOUS")),
