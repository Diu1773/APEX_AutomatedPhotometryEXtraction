...
from apex.analysis.light_curve.raw_lightcurve import (
    ...
    _build_source_to_id_map,
    _load_check_star_meta_by_filter,
    _active_comparison_ids_for_filter,
    _date_from_dateobs,
    _extract_date_from_path,
    _load_headers_map,
    _load_headers_table,
    _load_night_assignments,
    _load_selection_ids_by_filter,
    _load_target_radec,
    _resolve_fits_path,
    _select_rows_by_source_id,
)
...
class LightCurveBuilderWindow(StepWindowBase, RawLightCurveBuilder):
    ...
    # -- progress hooks: the window half of the preload seam ------------------

    def _preload_progress(self, index: int, message: str) -> None:
        if QThread.currentThread() != self.thread():
            self.background_worker_status.emit(index, message, "Loading", 0)
        else:
            self._worker_panel.update_worker(index, message, "Loading", 0)
