...
## 4.9 dark 를 바로잡고 다시 돌렸다 — 주기는 그대로다 (2026-07-31 밤)

다른 세션이 **YZ Boo 의 dark 매칭 결함**을 찾아 고치고 Step 0 을 재처리했다
(`8cd2f7f`, `a6af709`). night 20250430 의 283장이 −5 °C 에서 찍혔는데 −10 °C
dark 를 빼고 있었고(ΔT 5.0 °C), **그 283장에 위 §4.8 의 g 97장이 포함**된다.
원인은 자료가 아니라 코드였다 — `group_for_night` 이 같은 밤 dark 가 하나라도
있으면 전역 라이브러리를 통째로 배제해서 `E:\darks\dark-30s-5` 를 막았다.

재보정본(ΔT 0.16 °C)으로 Step 1–7 과 LC 를 **같은 절차로** 다시 돌렸다.
비교성은 이전과 같은 기준(커버리지 90%+ · σ 작은 순)으로 골랐고, 등급이
일치해 같은 별임을 확인했다(119 ≡ 이전 184 · 258 ≡ 369 · 304 ≡ 428).

| | −10 °C dark | **−5 °C dark (정정)** |
|---|---:|---:|
| 검출 소스 | 47,930 | 47,373 (−1.2%) |
| WCS-QC | 364/364 | 364/364 |
| **마스터** | **1,511** | **1,357** (−10.2%) |
| detected / forced | 46,599 / 503,405 | 46,386 / **447,562** (−11.1%) |
| 투명도 QC FAIL | 0 | 0 |
| 타깃 커버리지 | 364/364 | 364/364 |
| 시간축 | JD (BJD 가 NaN) | **BJD_TDB 364/364** |
| **최적 주기** | 0.104546 (+0.44%) | **0.104550 (+0.44%)** |
| power | 0.7877 | 0.7879 |
| 1위−2위 간격 | 0.8% | 0.72% |

**주기는 소수점 다섯째 자리까지 같다.** dark 온도를 5 °C 틀리게 뺐어도 이
결론은 안 흔들렸다 — 재처리 쪽 실측(밝은 픽셀 median Δ = 0.000, 변화가 hot
pixel 꼬리에만)과 정확히 일치한다.

**마스터가 10% 줄어든 것**은 hot pixel 이 제대로 마스킹되어 가짜 소스가
걸러진 결과로 보인다(−10 °C 마스크는 −5 °C 의 실제 hot pixel 29.4% 를 놓쳤다).
강제 측광이 11% 줄어든 것도 마스터를 따라간 값이다.

> **비교성 MAD 는 비교하지 않았다.** 이전 2밤 실행의 산출물이 재처리 때 함께
> 정리돼(`result/` 통째) 별 단위로 맞댈 대상이 없고, 필터를 섞어 계산하면
> g/r/i 영점 차이가 그대로 들어가 숫자가 무의미하다. 필터별로 다시 재려면
> 같은 조건의 대조군을 새로 만들어야 한다.
