    ...
    try:
        synth_clust = synth.generate(
            {"met": float(met), "loga": float(loga), "Av": float(av),
             "DR": 0.0, "Rv": 3.1, "dm": float(dm)}, N_stars=N_SYNTH)
...
t0 = time.time()
res = differential_evolution(cost, BOUNDS, seed=SEED, maxiter=60, popsize=12,
                            tol=0.01, polish=False, disp=True)
elapsed = time.time() - t0
met, loga, av, dm = res.x
