...
What the reader should get in one look:
  ...
  * the small red label under each step is the subsection that validates it — the
    map between what the tool does and where this paper checks it.

Monochrome with a single accent, per the paper figure convention.
...
SHARED = [("0  Calibration", "3.2–3.5"), ("1  File selection", "—"),
          ("2  Crop", "—"), ("3  Sky QC", "3.16"),
          ("4  Detection", "3.6"), ("5  WCS solve", "3.7"),
          ("6  Master cat.", "3.6"), ("7  Photometry", "3.8–3.11")]
CMD = [("8  PSF phot.", "3.12"), ("9  Master IDs", "—"), ("10  Zeropoint", "3.13"),
       ("11  CMD", "3.14"), ("12  Isochrone", "n/a")]
LC = [("8  Target sel.", "—"), ("9  Light curve", "3.15"),
      ("10  Detrend", "3.15"), ("11  Period", "§4")]
...
def box(ax, x, y, w, h, label, sec, fill, fs=7.0):
    ...
    if sec:
        ax.text(x + w / 2, y - 0.028, sec, ha="center", va="center", fontsize=6.0,
                color=ACCENT if sec not in ("—", "n/a") else MUT, zorder=3)
...
def main() -> int:
    ...
    ax.text(0.845, 0.912, "— the code Section 3 tests", fontsize=7.0,
    ...
    ax.text(0.045, 0.022,
            "red label = the subsection validating that step",
