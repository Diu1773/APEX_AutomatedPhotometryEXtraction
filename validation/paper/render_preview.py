...
FIGFILES = {
    ...
    12: "fig4_crosscheck_sep.png",           # 3.9 합성 독립 엔진
    13: "fig_iraf_crosscheck.png",           # 3.9 실측 독립 엔진(IRAF 499별)
    14: "fig_psf_validation.png",            # 3.10 PSF 측광
    15: "fig9_crowded_field.png",            # 3.11 밀집장
    16: "fig7_reference_crosscheck.png",     # 3.12 외부 목록(PS1)
    17: "fig8_cmd_reproduction.png",         # 3.12 CMD 재현
    18: "fig_timeseries_validation.png",     # 3.13 시계열 (SYSREM·PDM)
...
CAPTIONS = {
 2: r"검출기 보정(0단계) 검증. \textbf{(a)} 실제 M13 $V$ 프레임의 보정 전후와, 나눠지는 마스터 flat의 반경 프로파일(가장자리–중심 6\% 기울기). \textbf{(b)} 알려진 bias·dark·flat을 주입해 0단계로 되돌린 잔차: 계통 오프셋 $-0.004$ DN, 산포 MAD $3.13$ DN으로 주입한 읽기잡음 바닥($3.0$ DN)과 같은 크기다. \textbf{(c)} 선택적 우주선·핫픽셀 단계는 주입 우주선 화소의 100\%와 핫픽셀의 94\%를 지우면서 별 중심은 0.000\%만 건드리고 구경 플럭스를 0.00 mmag 바꾼다. \textbf{(d)} 같은 raw를 별개 코드베이스인 AstralImage/AIPPI 엔진으로 처리하면 자료 19벌에서 보정 프레임이 비트동일하고 마스터 프레임도 RMS $\le0.30$ DN으로 맞는다.",
