...
CAPTIONS.update({
  ...
  7: r"실측 영상 일곱 장에 경험적 PSF로 인공별을 주입한 검출 완전도. **(a)** 세 대표 단일 노출의 등급 공간 회수율과 $m_{50}$ (14.90–17.65등급); 점은 Wilson 95\% 구간이며 회색 파선은 합성 verification이다. 아래 스트립은 가장 얕은 M13 $V$ 프레임의 주입 컷아웃이다. **(b)** 이 그림의 주 결과로, 모든 주입별을 기대 peak-pixel S/N으로 바꾼 일곱 프레임이 한 곡선으로 모이며 합동 오차함수 적합은 $\mathrm{S/N}_{50}=4.0$, 프레임별 판독은 $4.05\pm0.18$이다.",
  8: r"검출 문턱값에 따른 헛검출 오염. **(a)** 실제 영상의 부호를 뒤집은 검출로 얻은 프레임별 안전 하한과 기본값 $3.2\sigma$의 오염률(다섯 영상 모두 2\% 이하). **(b)** Gaia DR3와의 독립 대조에서 목록 없이 추정한 헛검출 수가 44개 자료점에서 실측값의 2배 이내에 드는지 보인다. 부호반전은 대칭 잡음을 세는 상한이며 우주선·핫픽셀 같은 양의 결함은 별도 단계의 대상이다.",
 ...
 10: r"합성 몬테카를로 자료로 점검한 강제 구경측광 오차 모형. **(a)** 평균 편향은 $m\le17$에서 5 mmag 이하이고 어두운 쪽에서 등급 변환 비선형성이 커진다. **(b)** 구간별 RMS는 CCD 관계식 $1.0857/\mathrm{SNR}$를 따른다. **(c)** S/N $>5$ 표본의 pull 평균과 표준편차는 각각 $-0.148$, 1.014다. **(d)** 보고 오차 대 경험 RMS의 중앙비는 0.98이며 $m=19$에서 부족이 17\%로 가장 크다. 이 그림이 측광 오차 모형의 주 검증이고 그림 7-b는 검출 완전도의 주 검증이다.",
...
body{font-family:var(--serif);font-size:11.8px;line-height:1.34;color:var(--ink);
