...
FIGFILES = {
    1:  "fig10_calibration.png",            # 3.2 검출기 보정
    2:  "fig11_detector.png",               # 3.3 검출기 특성화
    3:  "fig12_preproc_crosscheck.png",     # 3.4 ccdproc 교차
    4:  "fig13_cross_instrument.png",       # 3.5 교차기기 보정
    5:  "fig_completeness_realvssynth.png", # 3.6 완전도 — 실측 주입판(옛 합성판 대체)
    6:  "fig2_error_model.png",             # 3.8 오차 모형
    7:  "fig3_parameter_sweep.png",         # 3.9 민감도
    8:  "fig4_crosscheck_sep.png",          # 3.10 합성 독립 엔진
    9:  "fig_iraf_crosscheck.png",          # 3.11 실측 독립 엔진 — NGC6811 499별(옛 NGC457판 대체)
    10: "fig9_crowded_field.png",           # 3.12 밀집장
    11: "fig7_reference_crosscheck.png",    # 3.13 참조 목록
    12: "fig8_cmd_reproduction.png",        # 3.14 CMD 재현
    13: "fig6_qc_validation.png",           # 3.16 프레임 QC
    14: "fig_lc_yzboo.png",                 # 4   과학 적용(YZ Boo)
}
...
CAPTIONS = {
 ...
 9: r"APEX가 raw에서부터 전부 줄인 NGC 6811 $V$ 프레임의 실제 별 499개를, 표 2에 맞춘 파라미터로 APEX 강제 측광과 IRAF `phot`(DAOPHOT)이 **같은 고정 좌표에서** 각각 잰 결과: MAD $9.7$ mmag, $r=0.99989$, 구간 중앙값 잔차가 어두운 쪽까지 평평하다 \citep{schechter1993}. 양쪽 모두 재중심을 껐으므로 잔차가 중심 잡기의 차이를 흡수할 수 없다. 이 불일치는 두 코드가 스스로 보고한 형식 오차의 제곱합근($27.8$ mmag)의 3분의 1 남짓이다.",
