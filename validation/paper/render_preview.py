...
@media print{
  ...
  /* 794×1123 CSS px rounds slightly larger than physical A4 in Chromium.
     Without the exact print override the browser fragments a bottom float
     onto the following sheet even though it is inside the viewer page. */
  .page{width:210mm!important;height:297mm!important;overflow:hidden!important;
...
(function(){
  ...
  var pages=[], pending=[], scale=1, sectionSerial=0;

  function el(t,c){ var e=document.createElement(t); if(c) e.className=c; return e; }
  function carrySource(from,to){
    if (from && from.dataset && from.dataset.srcBlock!==undefined)
      to.dataset.srcBlock=from.dataset.srcBlock;
    return to;
  }
  ...
  function contentBottom(c){
    ...
    if (!k) return 0;
    // offset geometry is invariant under the viewer's CSS zoom transform;
    // getBoundingClientRect mixed scaled pixels with unscaled clientHeight and
    // produced false "empty column" warnings at 50–99% zoom.
    return k.offsetTop-c.offsetTop+k.offsetHeight;
  }
  /* 넓은 float 직전에서 절이 끝나면 남은 본문이 왼쪽 단에만 몰린다.
     다음 절을 그림 앞으로 끌어오지 말고, 같은 절의 블록 경계만 바꾸어 두
     단의 높이 차가 가장 작은 지점을 고른다. */
  function balanceColumns(P){
    if (!P) return;
    var left=P.cols[0], right=P.cols[1],
        kids=[].slice.call(left.children).concat([].slice.call(right.children));
    if (kids.length<2) return;
    var best=kids.length, bestScore=Infinity;
    for (var split=1;split<kids.length;split++){
      kids.forEach(function(ch,i){ (i<split ? left : right).appendChild(ch); });
      if (!fits(left) || !fits(right)) continue;
      var lh=contentBottom(left), rh=contentBottom(right);
      var orphan=isHead(left.lastElementChild) ? 500 : 0;
      var score=Math.abs(lh-rh)+orphan;
      if (score<bestScore){ bestScore=score; best=split; }
    }
    kids.forEach(function(ch,i){ (i<best ? left : right).appendChild(ch); });
  }
  ...
  function resetFloatSize(node){
    var im=node && node.querySelector && node.querySelector('img');
    if (im){ im.style.maxHeight=''; im.style.width=''; }
  }
  function forceSizeTo(node, sp, cap){
    var old=draining; draining=true;
    var ok=sizeTo(node,sp,cap);
    draining=old;
    return ok;
  }
    ...
    while (pending.length){
      ...
      if (sp.offsetHeight>cap){
        if (wasEmpty && sizeTo(f,sp,cap)){
          ...
          if (sp===P.span){
            P._lockTopFloat=true;
            P._topFloatSection=parseInt(f.dataset.sectionSerial||'-1',10);
          }
        }
  ...
  function splitList(list, P){
    var tag=list.tagName.toLowerCase(), box=carrySource(list,el(tag,list.className));
    ...
    for (var i=0;i<kids.length;i++){
      ...
      if (!fits(P.cols[P.ci])){
        ...
        P=advance(P); box=carrySource(list,el(tag,list.className));
  ...
  function giveBackSpan(node, P, c){
    ...
    // An immediately placed top float is already in source order.  Returning
    // it to the queue merely to fit a later paragraph recreates the original
    // cross-section drift.  Bottom floats may still be yielded when needed.
    var sps=P._lockTopFloat ? [P.spanB] : [P.spanB, P.span];
  ...
  function place(node, P){
    if (isHead(node)) sectionSerial++;
    // Wide figures and tables are deterministic source-order floats.  First
    // try the foot of the current sheet; if it does not fit, put it at the top
    // of the next sheet immediately and let following prose continue below.
    // An unbounded queue was the cause of Fig. 3 appearing above §3.6 and
    // Figs. 14–17 being dumped after the manuscript.
    if (isSpanBlock(node)){
      node.dataset.floatBlock='1';
      node.dataset.sectionSerial=String(sectionSerial);
      // Drain only legacy give-back entries.  Normal wide floats never wait:
      // this bounds displacement to the immediately following sheet.
      if (pending.length){
        balanceColumns(P);
        while (pending.length) P=fresh();
      }
      var hasText=P.cols.some(function(c){ return c.children.length>0; });
      // A&A-style pages may carry one float at the head and a second at the
      // foot with a short bridge of prose between them.  Requiring the head
      // to be empty created four half-blank sheets before the next table or
      // figure (Figs. 4, 9, 12 and 14 in the current manuscript).
      var sameTopSection=!P.span.firstChild || P._topFloatSection===sectionSerial;
      var mayUseBottom=hasText && !P.spanB.firstChild && sameTopSection;
      if (mayUseBottom){
        balanceColumns(P);
        if (P.cols[1].children.length) P.ci=1;
        var bsp=P.spanB, bcap=spanCap(P,bsp);
        bsp.appendChild(node);
        forceSizeTo(node,bsp,bcap);
        if (bsp.offsetHeight<=bcap+1 && P.cols.every(fits)){
          // A wide float may sit at the foot while later prose from the same
          // subsection fills the columns above it.  Moving the active cursor
          // to the right column after balancing preserves reading order.
          P._bottomUsed=true;
          return P;
        }
      ...
      var pageEmpty=!hasText && !P.span.firstChild && !P.spanB.firstChild;
      if (pageEmpty){
        P.span.appendChild(node);
        var topCap=P.inner.clientHeight*(node.classList.contains('tw') ? 0.82 : 0.64);
        forceSizeTo(node,P.span,topCap);
        P._topFloat=true; P._lockTopFloat=true; P._topFloatSection=sectionSerial;
        return P;
      }
      // Close and balance the prose already read, then place the evidence at
      // the next sheet head.  Later prose uses the columns below this float,
      // so a half-height figure no longer creates a mostly empty page.
      balanceColumns(P);
      P=newPage();
      P.span.appendChild(node);
      var cap=P.inner.clientHeight*(node.classList.contains('tw') ? 0.82 : 0.64);
      forceSizeTo(node,P.span,cap);
      P._topFloat=true; P._lockTopFloat=true; P._topFloatSection=sectionSerial;
      return P;
    }
    // The next subsection must not begin visually above the previous
    // subsection's foot float.
    if (P._bottomUsed && isHead(node)){
      balanceColumns(P);
      P=newPage();
    }
    if (pending.length && isHead(node)){
      // Never allow a figure/table to cross a section boundary.  The previous
      // renderer sometimes moved the heading into an empty right column and
      // left the evidence figure several sections behind.
      balanceColumns(P);
      while (pending.length) P=fresh();
    }
  ...
  function build(){
    ...
    book.innerHTML=''; pages=[]; pending=[]; draining=false; sectionSerial=0;
    ...
    [].slice.call(flow.children).forEach(function(k,sourceIndex){
      var clone=k.cloneNode(true);
      clone.dataset.srcBlock=String(sourceIndex);
      P=place(clone, P);
    });
    // Rebalance after every float has settled.  A later caption or paragraph
    // can otherwise make the active right column much taller than the split
    // chosen when the foot float first arrived.
    pages.forEach(balanceColumns);
    ...
    renderNotes(); markAll();
    // 조판 자체 점검. 넘치는 단, 빈 왼쪽 단, 소스 순서 역전과 누락을
    // 각각 센다. 문단 분할은 같은 srcBlock을 여러 번 만들 수 있으므로
    // DOM 자식 수가 아니라 원본 인덱스의 유일 집합으로 검증한다.
    var over=0, holes=0, orderErrors=0, figureOrderErrors=0, floatBoundaryErrors=0,
        sourceOrder=[], seenSource={}, sourcePage={}, visualFigures=[];
    pages.forEach(function(p,pageIndex){
      if(!p.cols) return;
      p.cols.forEach(function(c){
        if (c.offsetParent!==null && c.scrollHeight>c.clientHeight+2) over++;
      });
      var leftUsed=contentBottom(p.cols[0]);
      var rightUsed=contentBottom(p.cols[1]);
      if (p.cols[1].children.length && p.cols[0].children.length &&
          leftUsed < p.cols[0].clientHeight*0.62 &&
          rightUsed>leftUsed+Math.max(120,p.cols[0].clientHeight*0.28)) holes++;
      if (pageIndex>1 && pageIndex<pages.length-1 && p.cols[0].children.length &&
          !p.cols[1].children.length && !p.spanB.firstChild &&
          leftUsed < p.cols[0].clientHeight*0.82) holes++;
      [p.span,p.cols[0],p.cols[1],p.spanB].forEach(function(region){
        if (!region) return;
        [].slice.call(region.children).forEach(function(ch){
          var raw=ch.dataset && ch.dataset.srcBlock;
          if (raw===undefined) return;
          var idx=parseInt(raw,10);
          if (!isFinite(idx)) return;
          seenSource[idx]=true;
          if (sourcePage[idx]===undefined) sourcePage[idx]=pageIndex;
          if (ch.dataset && ch.dataset.floatBlock==='1') return;
          if (!sourceOrder.length || sourceOrder[sourceOrder.length-1]!==idx)
            sourceOrder.push(idx);
        });
      });
      [].slice.call(p.el.querySelectorAll('figure img')).forEach(function(im){
        var fm=/그림\s*(\d+)/.exec(im.alt||'');
        if (fm) visualFigures.push(parseInt(fm[1],10));
      });
    });
    for (var oi=1;oi<sourceOrder.length;oi++)
      if (sourceOrder[oi]<sourceOrder[oi-1]) orderErrors++;
    for (var fi=0;fi<visualFigures.length;fi++)
      if (visualFigures[fi]!==fi+1) figureOrderErrors++;
    // A legal float can move within its subsection, but it must never appear
    // before that subsection's heading or after the next subsection starts.
    var sourceKids=[].slice.call(flow.children);
    sourceKids.forEach(function(ch,idx){
      if (!isSpanBlock(ch)) return;
      var before=-1, after=-1;
      for (var bi=idx-1;bi>=0;bi--) if (isHead(sourceKids[bi])){ before=bi; break; }
      for (var ai=idx+1;ai<sourceKids.length;ai++) if (isHead(sourceKids[ai])){ after=ai; break; }
      var fp=sourcePage[idx], bp=before>=0 ? sourcePage[before] : -1,
          ap=after>=0 ? sourcePage[after] : pages.length;
      if (fp===undefined || fp<bp || fp>ap) floatBoundaryErrors++;
    });
    var want=flow.children.length, got=Object.keys(seenSource).length;
    if (over || holes || orderErrors || figureOrderErrors || floatBoundaryErrors ||
        pages.length<6 || got!==want){
      ...
      m.textContent='조판 오류: '+pages.length+'쪽 · 넘침 '+over+' · 빈 단 '+holes+
        ' · 본문 역전 '+orderErrors+' · 그림 순서 '+figureOrderErrors+
        ' · 절 경계 '+floatBoundaryErrors+
        ' · 원본 블록 '+got+'/'+want;
      document.body.appendChild(m);
      if (window.console) console.warn('[APEX] 조판 오류 '+m.textContent);
    }
    /* Chromium's print fragmenter may detach a flex child's foot float and
       prepend it to the next physical sheet.  The paged viewer has already
       computed and audited exact geometry, so freeze the three vertical
       regions only for the headless-PDF build.  This makes printed pages a
       faithful copy of the viewer instead of asking a second layout pass to
       reinterpret the flex boxes. */
    if (/(?:^|[?&])print=build(?:&|$)/i.test(location.search||'')){
      pages.forEach(function(p){
        var cols=p.el.querySelector('.cols');
        var frozen=[p.span,cols,p.spanB].map(function(region){
          return {region:region,top:region.offsetTop,height:region.offsetHeight};
        });
        // Read every box before taking any flex child out of flow.  Measuring
        // after the previous child became absolute collapses the next offset
        // to zero and makes table, prose and figure overlap in the PDF.
        frozen.forEach(function(box){
          var region=box.region, top=box.top, height=box.height;
          region.style.position='absolute';
          region.style.left='0'; region.style.right='0';
          region.style.top=top+'px'; region.style.height=height+'px';
          region.style.flex='none'; region.style.boxSizing='border-box';
  ...
  document.addEventListener('keydown', function(ev){
    ...
    if (ev.shiftKey && (k==='ArrowLeft' || k==='ArrowRight')){
      ev.preventDefault();
      var sx=(k==='ArrowRight' ? 80 : -80);
      if (stage && stage.scrollWidth>stage.clientWidth+1){
        stage.scrollLeft=Math.max(0, Math.min(stage.scrollWidth-stage.clientWidth, stage.scrollLeft+sx));
      } else {
        window.scrollTo({left:Math.max(0,(window.scrollX||0)+sx), behavior:'auto'});
      }
      return;
    }
    if (k==='a' || k==='A' || k==='d' || k==='D'){
      ev.preventDefault();
      var sx2=(k==='d'||k==='D') ? 80 : -80;
      if (stage && stage.scrollWidth>stage.clientWidth+1){
        stage.scrollLeft=Math.max(0, Math.min(stage.scrollWidth-stage.clientWidth, stage.scrollLeft+sx2));
      } else {
        window.scrollTo({left:Math.max(0,(window.scrollX||0)+sx2), behavior:'auto'});
      }
      return;
    }
