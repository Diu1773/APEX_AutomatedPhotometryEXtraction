...
CAPTIONS = {
 ...
 5: r"(위) 몬테카를로 300시행, 인공별 $21{,}000$개 주입–되찾기(injection–recovery)로 잰 검출 완전도. 주입이 많아 등급 구간별 되찾음 비율 자체가 매끄러운 곡선이 되므로 함수를 맞추지 않고 깊이를 읽어 낸다(DES Balrog·AutoPhOT 관행; \citealt{anbajagane2025, brennan2022}): $m_{90}=17.34$, $m_{50}=17.60$, $m_{10}=17.84$등급. 고립/필드 배치의 50\% 깊이 차는 $0.04$등급(배치 강건); 필드 평탄부 $\approx88\%$는 기존 별과의 겹침 손실. (아래) 같은 프로덕션 코드로 심은 별의 컷아웃 — $m=17.5$(50\% 깊이 부근)는 눈으로도 경계선상, $m=18.3$은 사라진다.",
