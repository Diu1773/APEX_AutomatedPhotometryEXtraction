...
FIGFILES = {
    ...
    7:  "fig_completeness_realvssynth.png",  # 3.4 완전도(실측 주입)
    8:  "fig_detection_threshold.png",       # 3.5 검출 문턱·헛검출 오염
...
CAPTIONS.update({
  ...
  2: r"0단계 보정 자료와 보정 효과. 마스터 bias 8장, 60초 dark 8장, flat 5장으로 NGC 6811 $B$ 60초 영상을 보정했다(Moravian C3-61000, $2\times2$, 2026-06-11). flat 보정 뒤 하늘 배경의 좌우 진폭은 12.9\%에서 1.6\%로 줄었다.",
  3: r"광자전달곡선으로 측정한 검출기 상수. gain은 $0.681$ e$^-$/ADU, 읽기잡음은 $2.35$ e$^-$, 암전류는 $0.0077$ e$^-$/s이며 암전류 선형 적합의 $R^2$는 0.998이다. FITS 헤더의 EGAIN 값은 측정값보다 약 16배 작다.",
  4: r"APEX와 ccdproc의 보정 연산 대조. ① 마스터 bias·dark·flat 구성과 세 보정의 개별 적용은 모든 화소에서 비트 동일하다. 0인 값은 열린 원으로 표시 위치만 확보했다. ② 전체 연산 뒤 최대 차이는 $8.6\times10^{-4}$ DN으로, 읽기잡음 3.5 DN과 하늘 산탄잡음 41 DN보다 작다.",
  5: r"서로 다른 두 LCO 카메라에서 APEX 보정 영상과 BANZAI 산출물을 비교한 결과. QHY600 CMOS의 차이는 거의 균일한 $+0.06$ e$^-$이며, 4증폭기 Sinistro CCD의 화소값은 약 0.3\% 범위에서 일치한다.",
  6: r"주입 결함으로 구성한 44장 자료에서 자동 프레임 QC의 판정. 정상 24장은 모두 통과했고, 나쁜 시상·밝은 하늘·실제 읽기잡음과 헤더값이 다른 영상은 모두 REVIEW 또는 FAIL로 분류됐다. 광량만 0.7등급 낮춘 영상은 검출 단계 통계만으로 구분되지 않았다.",
  7: r"실제 영상 세 장에 경험적 PSF로 별을 주입해 측정한 검출 완전도. 등급으로 나타낸 $m_{50}$은 관측 조건에 따라 14.90–17.65등급이었으나, 주입별 peak-pixel S/N으로 바꾸면 세 곡선의 $\mathrm{S/N}_{50}$은 $4.05\pm0.18$로 모였다. 점은 Wilson 95\% 구간이며 곡선은 합동 적합이다.",
  8: r"검출 문턱값에 따른 헛검출 오염. 실제 영상의 부호를 뒤집어 같은 검출기를 적용했으며, 부호가 바뀐 영상의 양의 검출을 잡음 기원의 상한으로 사용했다. 기본값 $3.2\sigma$는 다섯 영상 모두에서 오염률 2\% 이하였고, 영상별 5\% 상한을 만족하는 최소 문턱값은 $1.5–2.2\sigma$였다.",
  9: r"M13 60초 영상 8장에서 built-in, ASTAP, astrometry.net 해의 비교. 왼쪽은 동일한 화소 격자를 각 WCS로 변환했을 때 해 사이의 각거리, 오른쪽은 같은 Gaia DR3 대조 절차로 계산한 검출 위치 잔차다. 세 검출 목록과 입력 영상은 동일하다.",
 10: r"합성 몬테카를로 자료로 점검한 강제 구경측광 오차. $m\le17$에서 평균 편향은 5 mmag 이하였고, S/N $>5$ 표본의 pull 표준편차는 1.014였다. 보고 오차의 가장 큰 부족은 $m=19$에서 17\%였으며, 전체 구간의 보고 오차 대 경험 RMS 중앙비는 0.98이다.",
 11: r"파라미터와 관측 조건에 대한 민감도. ① FWHM 6 px 영상에서 등급 오차 MAD는 $0.80\times$FWHM에서 최소였고, 위치 RMSE의 중앙값은 0.255 px였다. ② 하늘 배경 50→1000 ADU에서 $m_{50}$은 18.38→17.06등급, MAD는 36→88 mmag으로 변했다. ③ FWHM 3→6 px에서 $m_{50}$은 18.07→16.96등급, MAD는 42→60 mmag으로 변했다.",
 12: r"독립 측광 엔진과의 대조. ① 합성 영상의 같은 중심과 구경에서 APEX와 SEP의 영점 정렬 뒤 잔차 MAD는 6.0 mmag, RMS는 9.7 mmag이었다. ② APEX로 보정한 NGC 6811 $V$ 영상의 고정 중심 499개에서 IRAF/DAOPHOT과 비교한 잔차 MAD는 9.7 mmag이었다. 실선은 보정 곡선이 아닌 구간별 중앙값이다.",
 13: r"세 카메라, 여섯 자료군, 67장, 90,201개 별에서 PSF 측광과 강제 구경측광의 별 단위 차이. 자료군별 MAD는 7–40 mmag이었다. M45 광시야 자료의 마지막 모서리 구간은 중앙부에 비해 $+28$ mmag 차이를 보였다.",
 14: r"구상성단 M5와 M13 중심부에서 PSF 측광과 강제 구경측광의 차이를 최근접별 거리로 나타낸 결과. 약 10 px의 분해 하한부터 수백 px까지 각 구간 중앙값은 0에서 0.04등급 이내였다. 이는 두 내부 방법의 비교이며 외부 절대 정확도나 10 px 아래의 미분해 겹침을 검증하지 않는다.",
 15: r"NGC 6811의 외부 목록 및 CMD 대조. ①‑② PS1에 대한 APEX와 Gaia 변환 기준의 밝기별 잔차를 $B,V$에서 비교했다. ③ APEX와 Gaia 변환값으로 만든 Johnson CMD 1,917개의 주계열 ridgeline RMS는 19 mmag이다. 이 패널은 CMD 산출물을 검증하며 등시선 모수의 유일성을 검증하지 않는다.",
 16: r"합성 시계열로 점검한 PDM과 SYSREM. PDM은 0.06–0.20일, 잡음 5–40 mmag의 288개 계열에서 주기를 복원했고 같은 계열의 Lomb–Scargle 결과와 비교했다. SYSREM은 비교성만으로 공통 성분을 구했을 때 주입 변광 진폭의 109\%를 유지했지만, 변광성을 해 계산에 포함하면 0.4\%만 남겼다.",
 17: r"YZ Boo의 LC 모드 종단 산출물. 2026-03-28의 $r$ 대역 80장, 5.2시간 자료를 위상 접기한 광도곡선과 Lomb–Scargle 주기도. 단일 밤 최고 봉우리는 0.1046일로 문헌값 0.10409일과 0.5\% 차이이며, 여러 밤을 합치면 관측창의 $+1$ cycle/day alias가 가장 높아진다."
})
...
(function(){
  ...
  function spanCap(P, sp){
    ...
    // 140px 하한에 걸려 지면당 한 장으로 돌아간다(2026-08-03 실측 최대 7쪽 밀림).
    // 자리당 46% + 두 자리 합 80%. 실측 비교(2026-08-03):
    //   합 72%          -> 아래 자리가 140px 하한에 걸려 최대 7쪽 밀림
    //   46/80 (이 값)   -> 최대 4쪽 · 평균 3.5쪽 · 그림 최소 204px
    //   밀림시 50/88 부스트 -> 그림이 커지면 본문이 단에서 밀려나 giveBack 순환이
    //                      늘고 오히려 최대 7쪽으로 후퇴. 키우는 쪽은 답이 아니다.
    var other=(sp===P.span ? P.spanB : P.span).offsetHeight;
    //   합 0.88 로 올려도 악화(4.3 -> 5.2쪽, 2026-08-03 재확인). 그림이 커지면
    //   본문 단이 좁아져 문단이 더 잘게 쪼개지고, 그만큼 참조가 뒤로 밀린다.
    var room=P.inner.clientHeight*0.80 - other;
    var base=P.inner.clientHeight*0.46;
  ...
  function sizeTo(node, sp, cap){
    ...
    if (target<140 && !draining) return false;       // 읽을 수 없게 줄이느니 다음 지면
  ...
  function flushOne(P, sp){
    ...
    while (pending.length){
      ...
      if (sp.offsetHeight>cap){
        if (wasEmpty && sizeTo(f,sp,cap)){ pending.shift(); }
