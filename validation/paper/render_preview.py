...
CAPTIONS = {
 ...
 5: r"(a) 등급에 대한 완전도 — 인공별을 주입해 실제 검출기로 되찾은 비율(점, Wilson 95\%)과 오차함수(error function) 검출확률 모형(곡선). $m_{50}=17.56^{+0.08}_{-0.07}$, 전이폭 $0.33$등급. (b) 같은 되찾음을 S/N으로 다시 그리면 단일 곡선 $\beta=\tfrac{1}{2}[1-\mathrm{erf}(z/\sqrt{2})]$에 모이며 50\%가 S/N $\approx7.4$ — 검출을 지배하는 것은 등급이 아니라 S/N이다 \citep{masci2011, kashyap2010, brennan2022}.",
