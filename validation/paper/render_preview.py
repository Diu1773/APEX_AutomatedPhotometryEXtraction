...
(function(){
  ...
  function place(node, P){
    ...
    if (isSpanBlock(node)){
      ...
      if (node.classList.contains('tw') && !pending.length && !P._bottomUsed && bothColsUsed){
        var tsp=P.spanB;
      ...
      if (node.tagName==='FIGURE' && !pending.length && !P._bottomUsed && bothColsUsed){
        var fsp=P.spanB;
  ...
  function build(){
    ...
    /* 그림·표 때문에 둘째 단이 비어 버린 쪽은 그 페이지에 한해 본문을
       전폭으로 흘린다. 양쪽 단에 실제 내용이 있으면 2단을 유지한다. */
    pages.forEach(function(p){
      if (p.el.querySelector('figure,.tw') && p.cols[1] && !p.cols[1].children.length){
