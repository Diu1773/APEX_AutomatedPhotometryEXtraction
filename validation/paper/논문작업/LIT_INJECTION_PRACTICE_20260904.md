...
# 최근 논문들도 IRAF 와 대조하는가 (2026-09-05)

사용자 물음: *"걍 iraf랑 다 비교해버려도 되나? ... 최근논문들은 그런가 모르겠네"*

목적이 APEX 와 같은 도구 논문만 연도순으로 세우면 흐름이 보인다.

| 연도 | 도구 | IRAF·DAOPHOT 대조 |
|---|---|---|
| 2017 | AstroImageJ | **있다** — IRAF · IDL · MaxIm DL. 다만 수치 없이 선언만 |
| 2022 | AutoPhOT | **있다** — DAOPHOT 대조 그림(Fig 15). 본문에 mmag 없음 |
| 2024 | STDWeb | **없다** — 검증 절 자체가 없음 |
| 2026 | PhoPS | **없다** — 대신 Gaia 측성 대조와 자료군 사이 일관성 |

**최근일수록 안 한다.**

## 가장 최근 것 — PhoPS (2026)

**Erece & Kilic (2026)**, *PhoPS: An automated photometric pipeline for
survey-era astronomy*, arXiv:2607.27414. 2026-03-23 접수, 2026-07-27 게재확정.
대상은 *"small- to medium-sized observatories and moving target observations"* 로,
APEX 와 같은 자리다.

검증 절(§4 Validation and results)이 하는 것은 이렇다.

- **DAOPHOT·IRAF 대조 없음.** 역사적으로 언급만 하고 자기 구현을 독립적으로 검증한다.
- **인공별 주입 없음.**
- **Gaia DR3 대조는 측성에만.** 840 장에서 매칭 141,672 개로 총 RMS 잔차를
  0.284″ → 0.241″(15.0 % 개선)로 낸다.
- **측광 오차는 내부 비교별로 보정한다.** 외부 카탈로그가 아니다.
- **자료군 사이 일관성** — *"provides consistent results across datasets obtained
  with different telescopes, detectors, and observing conditions."*

덧붙여 PhoPS 는 천체 검출에 **DAOStarFinder** 를 쓴다. photutils 에 옮겨진
DAOPHOT 의 FIND 다. **알고리즘은 안에 들어와 있고, 프로그램은 대조 상대가
아니다.** 이 두 가지가 갈린다는 점이 이 흐름을 설명한다.

## 다만 갈래를 구분해야 한다

**도구 논문**은 최근일수록 IRAF 대조를 안 한다. 그런데 **시험·과학 논문**은 지금도
한다 — Jang (2023) 이 DOLPHOT 과 DAOPHOT 을 같은 자료로 돌렸다.

**「IRAF 와 대조한다」가 낡아서 사라지는 것이 아니라, 도구를 소개하는 논문에서
검증 자체가 얇아지는 흐름으로 보인다.** STDWeb 은 아무 검증도 안 하고, PhoPS 는
측성만 외부 기준으로 잰다.

## 그래서 「IRAF 랑 다 비교해버려도 되나」의 답

**된다.** AutoPhOT 이 2022 년 A&A 에서 그렇게 했고 AstroImageJ 는 2017 년 AJ 에서
수치도 없이 그렇게 했다. 심사에서 문제된 흔적이 없다.

**다만 두 가지가 안 따라온다.**

첫째, **오차막대의 정당성이 안 따라온다.** 앞 절에서 정리한 대로 DAOPHOT 의 보고
오차를 두고 네 논문이 갈리고 하나는 방향까지 반대다.

둘째, **최근 흐름과 어긋난다.** 2024·2026 의 같은 갈래 논문 둘이 IRAF 대조를
안 하므로, 심사자가 「왜 그것을 기준으로 삼았나」를 물을 여지가 생긴다.

## 다만 IRAF 가 낡아서 사라지는 것은 아니다

「구식」이라는 인상과 실제 상태가 다르다.

- 기관 차원의 개발은 **2013 년에 멈췄다.**
- 그러나 **2024 년 1 월** NSF NOIRLab 의 CSDC 와 US NGO 가 **NOIRLab IRAF
  v2.18** 을 내놓았다. Gemini 이용자를 위한 안정적 자료처리 수단이고, DRAGONS 가
  모든 장비를 덮을 때까지의 다리 역할이라고 밝히고 있다.
- **커뮤니티 배포판이 유지되고 있다.** 최신 안정판이 **2.18.1, 2025-04-10** 이고,
  NOIRLab 의 수정이 커뮤니티 2.18 로 병합됐다.

**살아 있고 큰 천문대가 지금도 쓴다.** 그러므로 「대조 상대가 죽은 소프트웨어다」는
반론은 사실이 아니다.

## 그러면 최근 도구 논문에서 사라지는 것은 무엇인가

IRAF 대조가 아니라 **검증 자체**로 보인다. STDWeb 은 아무 검증도 하지 않고,
PhoPS 는 외부 기준을 측성에만 쓴다. 그러니 「최근 논문이 안 하니 나도 안 해도
된다」로 읽을 자리가 아니라, **검증을 하면 이 갈래에서 오히려 눈에 띄는
자리**로 읽는 것이 맞다.
