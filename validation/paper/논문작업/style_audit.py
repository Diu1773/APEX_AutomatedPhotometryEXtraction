...
# 구체적인 동사로 쓴다.
check("S3", "영어 구문 직역",
      r"몫으로 남|(?:장벽|조건|문제|과제|한계)(?:이|가|은|는)? (?:그대로 )?남|"
      r"[으로]로 이어진다|음을 보[인여](?:주는)? 사례|부족한 것은|필요한 것은 "
      r"|것으로 이어[진]|보여주는 대목|다름 아니",
      "추상명사+남다/이어지다 대신 누가 무엇을 하는지 쓴다")

# S2 — 측정값 없는 평가어 (2026-07-29 추가)
#
# 문체가 아니라 **주장**의 문제다. 근거가 없으면 더 자연스럽게 고칠 것이 아니라
# 삭제하거나 낮춰야 한다. 기능 소개 문헌으로 사용성·수행시간·산출물 품질까지
# 확장하지 않는다(하네스 §3).
check("S2", "근거 없는 평가어",
      r"연구 수준|효과적[이으]|강건[하함]|신뢰할 수 있[는다]|우수[하함]|충분[하함]히 정확",
