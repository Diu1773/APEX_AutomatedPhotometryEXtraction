...
|---|---:|---|---|---|
| Astronomy and Computing | 1.8* | SCIE† | 상시 | **최적. 1차 투고처** |
| PASP | — | SCIE† | 상시 | 높음. **상향 대안·ASTROPOP 선례** |
...
\* 1.8은 ScienceDirect 저널 페이지의 현재 표시값이다. PASP의 IF는 이 대조에서
공식 저널 안내 페이지에 표시된 값을 사용하지 않아 비워 두었다.
† 색인 표기는 기관의 최신 Web of Science 검색 결과로 투고 직전에 다시 확인한다.

**2026-08-05 선택.** 현재 원고는 새로운 천문 알고리즘보다 GUI로 연결한 자료 수집·
전처리·검출·측광·검증 파이프라인이 중심이다. Astronomy and Computing은 천문 자료의
분석·감소·시각화, 자료처리 파이프라인과 과학 소프트웨어를 명시적으로 다루므로 1차로
정했다. PASP는 소프트웨어·기기 논문을 받지만, 더 넓은 천문학적 독자와 기여를 전면에
세울 수 있을 때의 상향 대안으로 남긴다.
