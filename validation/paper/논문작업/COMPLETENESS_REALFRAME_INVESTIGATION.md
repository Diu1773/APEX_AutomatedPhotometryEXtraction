...
## 검출 수 QC 문헌 조사 (2026-07-26, ars-lit-review)
사용자 발상: "Step 4에서 배경·FWHM으로 예상 검출수를 알고 벗어나면 과검출로 잡는다."
문헌 지위를 축별로 확정 (bib 추가: `stetson1994`, `danieli2020`, `smtn002`).

**① 하늘+시상으로 깊이 예측 = 확립된 표준.** Rubin [[smtn002]]의 단일방문 깊이
`m5 = C_m + dC_m + 0.50(m_sky−21) + 2.5log10(0.7/FWHM_eff) + 1.25log10(t/30) − k_atm(X−1)`
→ 한계flux ∝ σ_sky·FWHM. **APEX `frame_qc.py`의 `depth_ref = σ_sky_e × FWHM`과 동일 스케일링.**
단 SMTN-002는 공식을 내부문서 LSE-40에 귀속하고 "LSE-40 수치는 쓰지 말라"고 명시 —
원논문 귀속이 없으므로 인용은 SMTN-002 자체로.

**② 검출 수를 프레임 게이트로 = 선례 있음, 단 하한만.** Dragonfly WFS [[danieli2020]] §3.3.4:
`NOBJ < 1000` / `MEDFWHM > 5.5px` / `MEDELLIP > 0.3` 세 게이트. §5.1 — **프레임의 40~60%를
폐기하며 대부분이 검출수 하한에서 탈락.** 직접 확인한 두 가지: **상한 게이트 없음**,
**예상 검출수와 비교하지 않음(절대 임계만)**. APEX의 `nsrc < 0.25×trend`가 같은 계열이고,
빠진 게 정확히 상한 쪽 → 사용자 발상의 절반은 선례 있고 절반(예상치 대비 + 상한)은 신규.

**③ union 마스터 = ALLFRAME이 선례.** [[stetson1994]]: MONTAGE II 중앙값 합성으로 우주선 제거 →
그 위에서 검출("spurious detections 없는 목록") → ALLFRAME은 master 목록에 속하고
**"프레임의 상당 비율에 나타나는" 것만 실제로 인정**. 이게 min-frames 확인의 선례.
동시에 APEX 취약점을 규정: ALLFRAME은 합성이 1차 방어인데 **APEX는 단일프레임 검출의
합집합이라 그 방어가 없음** → `ref_union_min_frames` 기본 1이 위험한 이유.

**④ 절대 검출수 예측 불가 = 문헌이 아는 사실.** 별계수 기울기 d log N/dm이 은위·필드 의존.
고은위 광학 ~0.14 보고치가 우리 M67 측정 **+0.14~0.16**과 일치. 성단(M5·M13·M37)은
+0.33~0.67 — 성단 광도함수가 급하므로 당연. **k 고정 불가는 우리 데이터의 한계가 아니라
물리** → ②는 그룹 상대로만 성립. 논문에 이 한계를 명시해야 방어됨.

**iPTF 확인 완료 (ar5iv HTML 경유, PDF 추출은 실패했었음)**: PTFIDE(Masci+2017)는
프레임당 후보 수 **상한이 없습니다.** §4.8.2가 차분영상 QA 지표를 두고 `-baddiff`·`-uglydiff`로
배경·PSF매칭 품질에 1차원 컷을 걸지만, **후보 수 자체는 기각 기준이 아닙니다.**
프레임별 QA 지표는 천문측정 RMS·FWHM·배경·영점·5σ 한계등급 — 즉 ①계열이며 개수 아님.
→ **상한 선례 없음이 확정**. 조사한 세 파이프라인(Dragonfly·iPTF·ZTF) 모두 하한 또는
다른 지표만 사용.

**포지셔닝**: ①③은 기존 계보에 편입, ②의 "예상 대비 + 상한"은 신규 기여.

## 초과검출 게이트 구현·실증 (2026-07-26)
- **정상 산포 실측**: 실프레임 194장 / 24필터그룹(M5·M13·M37·M67·NGC6811, 작업본+재처리본)에서
  `n_sources ÷ 그룹중앙값` 최대 **1.53배**, 99분위 1.49배. 1.53은 노출 혼합(30–480s) 그룹.
- **임계 결정**: REVIEW 1.8배 / FAIL 2.5배 — 관측 봉투 바깥. 마법 숫자 아님(위 실측 근거).
- **원본 계수 사용**: `n_raw_detections`(eb31035) 우선, 없으면 `n_sources` 후퇴.
  `n_sources`는 모양필터·6000상한 뒤 값이라 과검출이 정상처럼 보일 수 있음.
- **검증**: 실데이터 128프레임 **FAIL 0·경고 0**(오탐 없음). M5 r(중앙값 1110)에 주입 —
  1.5배→REVIEW, 2.0배→REVIEW, **3.3배(사용자가 기억한 4000개 수준)→FAIL(초과율 4.08)**.
- 산출 열 `n_sources_excess_ratio` 추가. 테스트 `tests/test_frame_qc_excess_detections.py` 9개.
