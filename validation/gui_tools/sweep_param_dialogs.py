...
둔다 — 「넘치는 창 0 개」는 **잰 다섯에 대한 말**이지 열둘 전부가 아니다.
    ...
    도구가 죽는다  3 · 5                 — **앱이 아니라 이 도구의 한계다**

3·5 번은 `QDialog.exec_` 을 가로챈 상태에서 Parameters 를 누르면 프로세스가
`0xC0000409`(스택 버퍼 오버런)로 죽는다. **가로채기 없이 그냥 열면 둘 다 멀쩡히
뜨는 것을 확인했으므로 앱 결함이 아니다.** 이 둘은 아직 못 본 채로 남아 있다.
