...
모든 통제를 건 상태(적합창 1.7·FWHM, 둘 다 유지한 별):

| 엔진 | 전체 유지 | 같은 별에서 bias | 같은 별에서 산포 |
|---|---:|---:|---:|
| APEX step8 (gated) | 22/25 (0.88) | +0.033 | 0.088 |
| IRAF ALLSTAR | 14/25 (0.56) | +0.004 | 0.065 |

읽히는 방향은 **"APEX 는 더 많이 답하고, 답한 것의 정확도는 별 13 개로는
구분되지 않는다"** 이다. 두 엔진이 서로 비슷한 자리에 있다는 뜻이며,
`n = 13` 에서는 그 이상 말할 수 없다.

**다음**: 주입을 수백 개로 늘려(`--injections`) 밝기×혼잡 칸마다 통계가 서게
한 뒤 다시 채점한다. 지금 표의 모든 칸이 별 2~13 개에서 나온다.
