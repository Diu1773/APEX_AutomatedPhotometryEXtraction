...
- **M13**: 오래된 구상성단은 RGB 형상이 [M/H] 를 강하게 잡아 **B−V 단독으로도
  ...
  → **2026-08-15 후속:** V−R 을 더해 봤지만 안 풀린다. 오차만 무너지고
  값이 그대로라 어긋남이 61σ·41σ 가 된다 — 축퇴가 아니라 영점 척도다.
  문서 맨 뒤 「색을 하나 더 넣으면」 절 참조.
...
## M13 축퇴에 색을 하나 더 넣으면 — 풀리지 않고 못박힌다 (2026-08-15)

「M13 단색 dm−E 축퇴는 V−R 추가로 풀리는지 확인」(2026-08-04 후속 ③)의 답이다.
같은 자료·같은 사전분포((m−M)₀ 14.40 ± 0.15)·같은 설정(32 워커 × 6,000, 둘 다
수렴)으로 색만 바꿔 적합했다. 적색화는 단일 모수로 표본추출되고 소광법칙
(`R_band·E(B−V)`)으로 전 밴드에 걸리므로, 두 색은 서로 다른 소광비를 통해 E 를
독립으로 구속한다 — 설계상 축퇴를 풀 수 있는 실험이다.

| 16/50/84 백분위 | B−V 단독 | B−V + V−R | 문헌 |
|---|---|---|---|
| [M/H] | −2.09 / **−1.56** / −1.54 | −1.57 / **−1.55** / −1.53 | −1.53 ✓ |
| 나이 [Gyr] | 8.56 / **13.34** / 14.41 | 12.83 / **13.66** / 14.53 | 12.5 ✓ |
| (m−M)₀ | 13.94 / **13.945** / 14.25 | 13.95 / **13.958** / 13.97 | 14.4 ✗ |
| E(B−V) | 0.095 / **0.097** / 0.153 | 0.091 / **0.093** / 0.095 | 0.02 ✗ |

**[M/H] 와 나이는 문헌으로 들어온다.** 오래된 구상성단의 RGB 형상이 금속량을
잡는다는 기존 결론의 재확인이고, 나이 구간이 사전 범위 전체(±2.9 Gyr)에서
±0.85 Gyr 로 좁아진다.

**거리지수와 적색화는 오차만 무너지고 값이 안 움직인다.**
(m−M)₀ = 13.958 ± 0.007 은 사전값 14.40 에서 **61 σ**, E(B−V) = 0.093 ± 0.002 는
문헌 0.02 에서 **41 σ** 다. 두 색이 서로 일치하면서 문헌과 어긋난다 —
표본추출의 축퇴가 아니라 **계통**이라는 뜻이다. 통계오차가 14~16 배 줄었는데
중앙값이 그대로면 남은 것은 자료 자체의 척도다.

**어긋남의 절반은 색 척도, 절반은 등급 척도다.**

| | |
|---|---|
| 과잉 적색화 E(B−V) | 0.093 − 0.02 = **0.073 mag** |
| 그에 딸린 소광 A_V = R_V·E | 3.1 × 0.073 = **0.226 mag** |
| 거리지수 부족분 | 14.40 − 13.958 = **0.442 mag** |
| 적색화로 설명되는 비율 | **51 %** |

과잉 적색화가 소광을 통해 거리지수를 정확히 절반만큼 끌어내린다. 나머지 절반은
등급 영점 쪽이다. 둘 다 부호가 같고 크기가 같은 자릿수다.

**이건 M67 에서 이미 겪은 것과 같은 종류다.** 그때 [M/H] rail 을 푼 것은 색을
늘리는 게 아니라 **전 밴드 영점을 한 표준계(MMJ93)에 앉히는 것**이었다
(이 문서 앞부분). M13 의 B·V·R 영점은 Gaia 변환 참조에서 오므로 같은 검사가
필요하다. **색을 더 넣는 것으로는 영점 척도를 고칠 수 없다** — 두 색이 같은
방향으로 밀려 있으면 서로를 검증하지 못한다. 그것이 이번 실험의 결론이다.

다음 검사: M13 을 표준장 측광으로 재앵커한 뒤 같은 적합을 반복
(`recalibrate_u_mmj93.py` 와 같은 경로). 그 전까지 M13 의 (m−M)₀·E(B−V) 는
**미해결**로 둔다. [M/H]·나이는 두 색 판을 인용해도 된다.

산출물: `cmd_isochrone`(두 색) · `cmd_isochrone_BV_converged`(단색)

### 그러면 M13 을 무엇에 재앵커하나 — 자동 탐색은 빈손이다 (2026-08-15)

위 결론이 지목한 다음 검사(표준장 재앵커)가 M13 에서 성립하는지 먼저 봤다.

**현재 앵커는 전부 Gaia 변환이다** (`zp_fit_coefficients.csv`):

| 밴드 | 영점 | 색항 | 참조 |
|---|---:|---:|---|
| B | −3.935 | +0.307 | **Pancino+2022 dwarf** |
| V | −3.435 | +0.021 | **Riello+2021** |
| R | −3.851 | −0.167 | **Riello+2021** |

M67 감사에서 MMJ93 대비 실측 오프셋이 나온 바로 그 앵커 부류다
(그때 dB = +0.051 로 Pancino 관계의 공칭 σ≈0.025 를 1~2 σ 초과).

**표준 카탈로그 자동 탐색은 빈손이다.** `discover_standard_catalogs` 를
`M13` · `NGC 6205` · `Messier 13` 세 이름으로 돌렸고, 키워드 후보 4 개가
전부 걸러졌다. 후보를 직접 열어 확인한 결과 걸러진 게 맞다:

| 후보 | 정체 |
|---|---|
| IV/26 | Ludendorff **1905** 사진등급 — 표준계 아님 |
| VII/118 · VII/1B · VII/239A | NGC/IC **위치** 카탈로그 — 측광 아님 |

즉 판정기의 위음성이 아니라 **VizieR 키워드 경로에 M13 현대 표준 측광이
없다**. NGC 6811 에서 「시야 내 표준성 없음」이 정답이었던 것과 같은 상황이다.

**남은 길은 키워드 검색 밖에 있다** — Stetson 균질 측광 표준장(CADC 호스팅)이
구상성단을 다수 포함하므로 M13 도 있을 가능성이 있으나, VizieR 키워드로는
안 잡히고 별도 취득 경로다. 그 취득 여부는 사용자 판단 사항으로 남긴다.

**그때까지의 정직한 서술**: M13 의 (m−M)₀ 와 E(B−V) 는 **영점 척도 문제로
규명됐으나 이 자료만으로는 보정할 수 없다.** [M/H] −1.55 와 나이 13.66 Gyr 는
두 색 판을 인용해도 된다(둘 다 문헌 범위).
