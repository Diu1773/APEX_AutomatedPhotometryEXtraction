...
시험 **1,444 개**가 세 운영체제와 파이썬 세 판에서 돈다(2026-09-06 기준).
...
**밀도 축을 열었다.** 별 400 → 1,600 → 6,400 개(구상성단 중심 수준, 2 초각 안에
이웃이 있을 확률 38 %)로 올려도 어긋난 것이 **둘, 곧 0.03 %** 다. 매칭이 반지름
안의 아무나가 아니라 가장 가까운 것을 고르기 때문이다. 그래서 **분할은 측성 오차 대
병합 반지름이 정하고, 병합은 밀도가 아니라 측성 오차가 정한다.**

**모드 축도 닫혔다 (2026-09-06).** `ref_build_mode` 의 두 값은 마스터 목록이 다
만들어진 뒤 붙이는 번호만 달랐다. 네 경우를 실제 엔진으로 돌려 마스터 400 · 분할 0 ·
미청구 0 이 모두 같은 것을 확인하고 **설정을 없앴다.**

**남은 것.** 이소크론의 거리지수 바닥은 원인 미상이다. 검출 완전성은 세 실험 모두
「시야 안의 별은 모든 프레임에서 검출된다」로 고정했고, 왜곡·오염도 한 가지다.

---

**앱 쪽에서 고친 결함 셋 (2026-09-06).** 측정하다 나온 것들이고, 막지 않고 원인까지
갔다.

| 무엇 | 어디 | 어떻게 됐나 |
|---|---|---|
| 관측일을 건너 별을 잇는 열쇠 | `apex/analysis/merge/id_match.py` | `source_id`(워크스페이스 순번일 수 있음) → `gaia_source_id`. 하늘의 정반대 두 폴더가 합쳐지던 것 |
| Gaia 가 없을 때의 번호 | `apex/analysis/refbuild.py` | 양수 순번 1..N 이 남던 것을 전부 음수로. 「양수면 Gaia」가 구성상 참이 됐다 |
| 19 자리 번호의 끝 세 자리 | `apex/utils/io_utils.py` 외 | float 열에 들어가 반올림되던 것. 읽기·구성·쓰기 세 자리를 Int64 로 |

**같은 작업에서 지운 것.** `RefBuildWorker` 의 안 불리는 **1,001 줄**(2,083 → 1,043).
엔진이 옮겨 간 뒤 남은 옛 사본이고, **Step 6 단위 시험 셋이 그 사본을 검사하고
있었다.** 실제로 도는 함수를 모듈 수준으로 꺼내고 시험을 옮겼다.

### 둘 — 다른 카메라에서도 되는가

**끝났다 (2026-09-08).** LCO MuSCAT3(2 m, ogg)의 M67 원본 180 장을 APEX 로
Step 0 부터 Step 11 까지 돌리고, 같은 성단을 Moravian C3-61000 으로 찍어 역시
APEX 로 처리한 결과와 등급을 견줬다. 세 곳(두 기기 + PS1) 모두에 있는 별 243 개로
표본을 맞췄다.
...
**보정 단계도 관측소 자신의 파이프라인과 맞았다** — BANZAI 와 화소 상관 0.9998,
32×32 블록에서 하늘의 0.16 ~ 0.22 %, 매끄러운 잔차 성분 0.

**나온 결함 하나는 뿌리까지 갔다.** r 밴드가 어두운 쪽에서 −0.27 휘는데, APEX 가
MuSCAT3 rp 에 2차 색항 −0.873 ± 0.146(6.0σ)을 적합해 놓고 **적용하지 않기 때문**
이다(D-012, 2026-08-21). Moravian 에서는 그 항이 거의 0 이라 무해했다.

자세한 것은 `validation/external_muscat3/` — `MAGNITUDE_ERROR_ANALYSIS.md` ·
`STEP0_VS_BANZAI.md` · `PTC_MuSCAT3.md`.
