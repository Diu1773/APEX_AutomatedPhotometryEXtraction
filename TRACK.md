...
### D-004 · `ref_union_min_frames` 기본값을 얼마로 할지

- 무엇: union 마스터에 별을 넣기 위한 최소 검출 프레임 수. 현재 기본값 1 (= 필터 없음).
- 지금: 실측이 나왔다 (위 「측정 완료」). M67 g′ 10장에서 마스터 766 소스 중
  1프레임 검출뿐인 것이 78개(10.2%), 10장 전부 검출이 560개(73.1%) — 뚜렷한 쌍봉.
- 선택지:
  **A. 2** — 1프레임짜리 78개(10.2%)만 정확히 잘라낸다. 두 번 이상 보인 것은 전부 보존.
  **B. 3** — 97개(12.7%) 제거. 경계 별 19개가 더 빠진다.
  **C. 1 유지** — 필터 없음. 마스터가 잡음 소스를 품는 대신 아무것도 안 버린다.
- 근거: 분포가 쌍봉이라 1프레임 검출은 질적으로 다른 집단이다(우주선·잡음·문턱 깜빡임).
  논문 인용 선례와도 맞는다 — `stetson1994`(ALLFRAME)의 "상당 비율 프레임에 나타나는
  것만 인정". 단, **이 실측은 한 밤·한 필터·한 시야(M67)다.** 밀도·깊이가 다른 시야에서
  10.2%가 흔들릴 수 있다 — 일반화하려면 M3(BVR 10장)로 한 번 더 재면 된다.
- 권고: **A (2).** 쌍봉의 골이 1과 2 사이에 있고, 잃는 것이 정확히 "한 번만 보인" 집단이다.
- 되돌리기: config 한 줄. 마스터는 재실행으로 재생성된다 (이번 실측 기준 detect 50 s + union 4 s).
